      ...
      module Notifications
        ...
        class Subscription
          ...
          def start_span(name, id, payload, start = nil)
            ...
            # Start a trace
            span_started = tracer.start_span(@span_name, @options)
            tracer.with_span(span_started) do 
              # Assign start time if provided
              span_started.start_time = start unless start.nil?
              payload[:otel_span] = span_started
            end
          end

          def finish_span(name, id, payload, finish = nil)
            payload[:otel_span].tap do |span|
          ...
          class Handler
            ...
            def run(span, name, id, payload)
              ...
              OpenTelemetry.logger.debug("ActiveSupport::Notifications handler for '#{name}' failed: #{e.message}")
            end
